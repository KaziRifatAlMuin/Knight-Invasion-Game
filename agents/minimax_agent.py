...
class MinimaxAgent:
    def __init__(self, player=2, depth=2, max_block_first=None, max_block_second=None):
        self.player = player
        self.depth = depth
    ...
    def choose_action(self, state):
        ...
        # Safe fallback: prefer a move if available, otherwise any legal action.
        move_actions = [action for action in actions(state, self.player) if action[0] == "move"]
        if move_actions:
            return move_actions[0]

        all_actions = actions(state, self.player)
        return all_actions[0] if all_actions else None


def actions(state, player, max_block_first=None, max_block_second=None):
    """Return every legal move or block action for the current player."""
    action_list = [("move", move) for move in state.get_moves(player)]

    # If the knight is adjacent to fire, the rules force a move.
    if state.must_move(player):
        return action_list

    # If no legal block exists, only moves are available.
    if not state.block_possible():
        return action_list

    first_cells = state.get_first_block_candidates(player)
    first_cells = rank_first_blocks(state, player, first_cells)

    if max_block_first is not None:
        first_cells = first_cells[:max_block_first]

    for first in first_cells:
        seconds = state.get_second_block_candidates(player, first)
        seconds = rank_second_blocks(state, player, first, seconds)
        if max_block_second is not None:
            seconds = seconds[:max_block_second]
...
def max_value(state, depth, alpha, beta, agent_player, current_player, max_block_first, max_block_second):
    ...
    for action in actions(state, current_player, max_block_first, max_block_second):
...
def evaluate(state, agent_player):
    ...
    agent_moves = len(state.get_moves(agent_player))
    opponent_moves = len(state.get_moves(opponent))
    agent_block_options = len(state.get_first_block_candidates(agent_player))
    opponent_block_options = len(state.get_first_block_candidates(opponent))
    agent_has_block = 1 if agent_block_options > 0 else 0
    opponent_has_block = 1 if opponent_block_options > 0 else 0

    # Positive score means better for the agent.
    distance_score = (opponent_dist - agent_dist) * 24
    mobility_score = (agent_moves - opponent_moves) * 8
    block_score = (agent_block_options - opponent_block_options) * 12
    pressure_score = (opponent_dist - agent_dist) * 14 if opponent_has_block else 0
    tactical_score = (agent_has_block - opponent_has_block) * 12
    progress_score = (row_progress(agent_pos, agent_player) - row_progress(opponent_pos, opponent)) * 2

    return distance_score + mobility_score + block_score + pressure_score + tactical_score + progress_score
